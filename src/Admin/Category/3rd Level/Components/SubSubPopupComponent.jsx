...
const SubSubPopupComponent = ({ token, isModalOpen, setIsModalOpen, id, fetchCategories }) => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      await Api.post(`/createSubSubCategory/${id}`, form, {
        headers: {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center backdrop-blur-md z-65" style={{ backgroundColor: "rgba(0, 0, 0, 0.25)" }}>
      <div className="bg-white p-6 rounded-lg shadow-lg w-96">
        ...
        <div className="flex justify-end space-x-4 mt-4">
          <button onClick={() => setIsModalOpen(false)} className="bg-gray-400  cursor-pointer text-white px-4 py-2 rounded-lg hover:bg-gray-500 transition">Cancel</button>
          <button onClick={handleSave} className="bg-[#795ded] text-white px-4 py-2 rounded-lg cursor-pointer transition">Add</button>
        </div>
      </div>
...
export default SubSubPopupComponent;
