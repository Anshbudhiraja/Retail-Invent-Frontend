...
const ShowPurchase = ({ totalPurchases, productId, onClose, token }) => {
  const [rates, setRates] = useState([]);
  const [loading, setLoading] = useState(true);
  const [shouldLogout, setShouldLogout] = useState(false);
  const [currentPage, setCurrentPage] = useState(0);
  const pageCount = totalPurchases.totalPages;
  ...
  const fetchRates = async (page) => {
    setLoading(true);
    try {
      const response = await Api.get(`/getAllPurchases/${productId}?page=${page}`, {
  ...
  return (
    <div className=" flex flex-col min-h-screen items-center">
      <div className="w-full max-w-lg  md:max-w-7xl">
        <h2 className="text-base font-bold text-center text-gray-800 mb-3">Purchase History</h2>
        {loading ? (
          ...
          <>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mt-6 px-10">
              ...
              <div className="font-spline_sans m-5 text-sm text-gray-600 dark:text-gray-400">
                Showing {currentPage + 1} of {totalPurchases.totalPages} Pages
              </div>
              <nav>
          ...
          ${currentPage === page ? "bg-[#795ded] text-white" : "text-gray-700 hover:bg-gray-200"}`}
                  ...
                  <li className="border border-gray-200 rounded-lg md:rounded-md">
                    <button
                      ...
                      className={`flex items-center justify-center w-9 h-9 md:w-7 md:h-7 rounded-lg md:rounded-md font-medium sm:text-sm md:text-xs transition-all duration-200
      ${currentPage === pageCount - 1 ? "text-gray-400 cursor-not-allowed" : "bg-[#795ded] text-white hover:bg-[#684edb]"}`}
          ...
          <p className="text-center text-red-500 font-medium mt-3">No History Found</p>
        )}
      </div>
