...
const ForgotOtp = ({email, cancel}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            console.log("Sending OTP verification request..."); // Debugging
            const response = await Api.post("/verifyForgetUser", { email, otp: otpValue });
    ...
    return (
        <div>
            {passwordToggle ? (
                <ForgotNewPass Token={Token} cancel={() => setPasswordToggle(false)} />
            ) : (
                <motion.div
